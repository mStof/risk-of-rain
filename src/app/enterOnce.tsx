...
const Enter = ({ children }: { children: ReactNode }) => {
  ...
  const enterTransition = contextSafe(() => {
  ...
  useEffect(() => {
   if (pathname === "/" ) enterTransition();
  },[enterTransition, pathname]);
  ...
  return (
    <>
      <div
        ref={bgElem}
        className="fixed h-svh w-full scale-x-100 z-90 bg-dark-10 bg-(image:--bg-patter) flex justify-center items-center p-60"
      >
